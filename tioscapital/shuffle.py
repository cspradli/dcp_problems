def shuffle(amount, total_amount, deck, flag1, flag2):
    ...
    print("%d / %d" % (total_amount - amount, total_amount))
    print(new_deck)
    if(flag2):
        ...
        for card in range(52):
            if new_deck[card] == 52 and (new_deck[card-1] == 1 or new_deck[card+1] == 1):
                print("Top and bottom are touching on shuffle number %d" % (total_amount - amount))
                return new_deck
    if(flag1 and new_deck[0] == 52):
        ## Flag 1 represents to look for 52 on bottom ##
        print("52 on bottom at shuffle number %d" % (total_amount - amount))
        return new_deck
